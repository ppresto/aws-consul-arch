...
module "eks-use1" {
  ...
  # cluster_additional_security_group_ids = [module.sg-consul-dataplane-use1[each.key].securitygroup_id]
  # node_security_group_id                = module.sg-consul-dataplane-use1[each.key].securitygroup_id
...
resource "local_file" "use1" {
  ...
  filename = "${path.module}/../consul-clients/auto-${local.use1[each.key].eks.cluster_name}.tf"
}
