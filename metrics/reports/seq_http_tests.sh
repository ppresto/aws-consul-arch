...
baseline(){
    ...
    do
        if [[ ${type} =~ consul_http ]]; then
            ...
            ${SCRIPT_DIR}/fortio_cli.sh  -j -t baseline_http -n fortio-baseline -d${DURATION} -w${WAIT_PERIOD} -c${c}
            pid=$!
            wait $pid
            echo "Test completed (PID: $pid)"
        else
            echo echo "Running Test Type 'baseline_grpc' in namespace fortio-baseline with $c connections"
            ${SCRIPT_DIR}/fortio_cli.sh  -j -t baseline_grpc -n fortio-baseline -d${DURATION} -w${WAIT_PERIOD} -c${c}
            pid=$!
            wait $pid
        fi
...
do
    ...
    do
        ...
        do
            ...
            ${SCRIPT_DIR}/fortio_cli.sh -j -t ${type} -n ${NAMESPACES[$ns]} -d ${DURATION} -w${WAIT_PERIOD} -c${c}
            pid=$!
            wait $pid
        done
        baseline ${c}
